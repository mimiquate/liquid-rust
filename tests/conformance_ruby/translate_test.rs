        ...
        let translation = match translations_config {
            Ok(parsed_translation_configs) => {
                ...
                match parsed_translation_configs.as_object() {
                    Some(translations_config_object) => {
                        resolve_translation(path, translations_config_object)
                    }
...
fn test_filter_translation() {
    let assigns = o!({"first_name": "John", "knock_translations_config": o!({"layout": o!({"header": "Oh my header"})})});
    //  "HI {{user.first_name}}"

    assert_template_result!(
        "Hello John, literal!",
        "{{ 'layout.hello_user' | t: name: first_name, other: 'literal' }}",
        assigns,
