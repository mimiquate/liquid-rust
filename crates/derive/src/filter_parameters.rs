...
fn generate_construct_keyword_list_field(field: &FilterParameter<'_>) -> TokenStream {
    ...
    quote! {
        let #name = Expression::with_object_literal(sape);
    }
}
...
fn generate_impl_filter_parameters(filter_parameters: &FilterParameters<'_>) -> TokenStream {
    ...
    let keyword_list_exist = keyword_list_fields.peekable().peek().is_some();
    ...
    let otro = if keyword_list_exist {
        quote! {
            {}
        }
    } else {
    ...
    let algo = if keyword_list_exist {
         keyword_list_fields_handling_blocks.first().unwrap().clone()
    } else {
        quote! {}
    };

    let keywords_handling_block =
        quote! {
            let mut sape: std::collections::HashMap<String, liquid_core::runtime::Expression> = std::collections::HashMap::new();
            ...
            while let ::std::option::Option::Some(arg) = args.keyword.next() {
                sape.insert(arg.0.into(), arg.1.clone());
                match arg.0 {
